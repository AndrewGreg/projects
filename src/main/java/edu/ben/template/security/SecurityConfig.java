...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/").permitAll().antMatchers("/register").permitAll()
				.antMatchers("/jobs").permitAll().antMatchers("/jobs/*").permitAll().antMatchers("/newEvents/*")
				.permitAll().antMatchers("/eventsTemplate").permitAll().antMatchers("/faculty").permitAll()
				.antMatchers("/testimonialList").permitAll().antMatchers("/jobSearch").permitAll()
